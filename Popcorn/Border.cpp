...
void AsBorder::Open_Gate(int gate_index, bool short_open)
{
	if (gate_index != Gates.size() - 1 && short_open)
		AsConfig::Throw(); // The platform can only be rolled out from the right international gate(Платформу можно выкатывать только из правого нижнего гейта)
...
{// Correcting the position when reflected from the frame ( Корректируем позицию при отражении от рамки)
	...
	if (AsConfig::Level_Has_Floor && next_y_pos + AsConfig::Ball_Radius > AsConfig::Floor_Y_Pos)
	{
		...
		if (ball->Get_State() == EBall_State::On_Parachute)
			ball->Set_State(EBall_State::On_Parachute);
	}
