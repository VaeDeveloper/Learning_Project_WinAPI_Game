...
const double AGate::Gap_Height_Long_Step = Max_Gap_Long_Height / ((double)AsConfig::FPS * 1.5);  // Для анимации за 3 секунды
//------------------------------------------------------------------------------------------------------------
AGate::AGate(int x_pos, int y_pos, int level_x_pos, int level_y_pos)
	...
	Origin_Y_Pos(y_pos), Edges_Count(5), Gate_Close_Tick(0), Level_X_Pos(level_x_pos), Level_Y_Pos(level_y_pos), Gap_Height(0.0)
{
