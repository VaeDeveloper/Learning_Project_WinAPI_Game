...
//------------------------------------------------------------------------------------------------------------
AsEngine::AsEngine()
	: Game_State(EGame_State::Enter_Name), Rest_Distance(0.0), Modules{}, Timer_ID(WM_USER + 1), Got_Name(false)
{
}
//------------------------------------------------------------------------------------------------------------
void AsEngine::Init_Engine(HWND hwnd)
{// Game setup at startup (Настройка игры при старте)
	int index;
	SYSTEMTIME sys_time;
	FILETIME file_time;
...
{// Отрисовка экрана игры
	...
	for (auto* curr_modules : Modules)
	{
		curr_modules->Clear(hdc, paint_area);
	}

	for (auto* curr_modules : Modules)
	{
		curr_modules->Draw(hdc, paint_area);
	}
}
//------------------------------------------------------------------------------------------------------------
...
{
	if (Platform.Has_State(EPlatform_Substate_Regular::Missing) && Monster_Set.Are_All_Destroyed())
		return true;
	else
		return false;
}
...
void AsEngine::On_Falling_Letter(AFalling_Letter* falling_letter)
{
	...
	AsInfo_Panel::Update_Score(EScore_Event_Type::Catch_Latter);
