...
int APIENTRY AsMain_Window::Main(HINSTANCE instance, int command_show)
{
	...
	hAccelTable = LoadAccelerators(instance, MAKEINTRESOURCE(IDC_POPCORN));
...
LRESULT CALLBACK AsMain_Window::Window_Proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		// function case paint double buffering graphics
	case WM_PAINT:
		Self->On_Paint(hWnd);
		break;
...
V 2.6. + (Переход на следующий уровень)
...
V 2. Состояния игры (заставка, анимация начала уровня, играем уровень, потеря жизни, переход на следующий уровень (нормальный и досрочный), окончание игры)


Дизайн
V 1.Изменить дизайн (цвета)
