...
//------------------------------------------------------------------------------------------------------------
AsMop::AsMop()
	: Mopping_Is_Done(false), Y_Pos(0), Max_Y_Pos(0), Lifting_Height(0), Start_Tick(0), Mop_State(EMop_State::Idle), Mop_Rect{}, Prev_Mop_Rect{}
{
	int i;
	int x_pos, y_pos;
...
void AsMop::Activate(bool cleaning)
{
	...
	if (cleaning)
	{
		...
		Lifting_Height = Get_Cylinder_Height() + Height;
...
void AsMop::Set_Mop()
{
	...
	Y_Pos = Max_Y_Pos - Get_Cylinder_Height() - Height + 1;
