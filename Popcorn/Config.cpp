...
AColor::AColor(unsigned char r, unsigned char g, unsigned char b)
	: R(r), G(g), B(b), Pen(CreatePen(PS_SOLID, 0, RGB(r, g, b))), Brush(CreateSolidBrush(RGB(r, g, b)))
{
}
//------------------------------------------------------------------------------------------------------------
AColor::AColor(const AColor& color, int pen_size)
	: R(color.R), G(color.G), B(color.B), Pen(CreatePen(PS_SOLID, pen_size, color.Get_RGB())), Brush(0)
{
}
//------------------------------------------------------------------------------------------------------------
AColor::AColor(unsigned char r, unsigned char g, unsigned char b, int pen_size)
	: R(r), G(g), B(b), Pen(CreatePen(PS_SOLID, pen_size, RGB(r, g, b))), Brush(0)
{
}
//------------------------------------------------------------------------------------------------------------
AColor::AColor(const AColor& pen_color, const AColor& brush_color, int pen_size)
	: R(0), G(0), B(0), Pen(CreatePen(PS_SOLID, pen_size, pen_color.Get_RGB())), Brush(CreateSolidBrush(brush_color.Get_RGB()))
{
}
//------------------------------------------------------------------------------------------------------------
...
{
	if (fade_step < 0 || (std::vector<AColor>::size_type)fade_step > Fading_Colors.size())
		AsConfig::Throw();
...
const AColor AsConfig::BG_Color(15, 15, 15);
const AColor AsConfig::Red_Color(255, 86, 255);
const AColor AsConfig::Blue_Color(0, 255, 255);
const AColor AsConfig::White_Color(245, 245, 245);
...
const AColor AsConfig::Monster_Dark_Red_Color(199, 21, 133);
...
const AFont AsConfig::Name_Font(-44, 700, 34, L"Consolas");
const AFont AsConfig::Score_Font(-44, 700, 49, L"Consolas");
const AFont AsConfig::Logo_Pop_Font(-128, 900, 34, L"Arial Black");
const AFont AsConfig::Logo_Clone_Font(-85, 900, 34, L"Arial Black");
const AFont AsConfig::Game_Over_Font(-85, 900, 40, L"MV Boli", true);
...
{
	for (auto* hit_chechers : Hit_Checkers)
		if (hit_chechers->Check_Hit(x_pos, y_pos, ball))
			return true;
...
{
	for (auto* hit_chechers : Hit_Checkers)
		if (hit_chechers->Check_Hit(x_pos, y_pos))
			return true;
...
{
	for (auto* hit_chechers : Hit_Checkers)
		if (hit_chechers->Check_Hit(rect))
			return true;
