...
AsLevel::AsLevel()
	...
	Parachute_Color(AsConfig::Red_Color, AsConfig::Blue_Color, AsConfig::Global_Scale), Advertisement(0), Current_Brick_Left_X(0),
	Current_Brick_Right_X(0), Current_Brick_Top_Y(0), Current_Brick_Low_Y(0), Current_Level{}
{
	Level = this;
}
//------------------------------------------------------------------------------------------------------------
...
{// Вывод всех объектов уровня
	...
	if (IntersectRect(&intersection_rect, &paint_area, &Level_Rect))
	{
		...
		Mop.Clean_Area(hdc); // Очищаем часть,стертую шваброй
	}
...
void AsLevel::Init()
{
	...
	for (i = 0; i < ALevel_Data::Max_Level_Number; i++)
	{
		...
		if (i == 7)// номер уровня для рекламы && если надо добавить 
			level_data->Advertisement = new AAdvertisement(4, 7, 2, 3); // Добавляем рекламу в уровень в котором есть уровень
...
void AsLevel::Set_Current_Level(int level_number)
{
	...
	if (level_number < 1 || (std::vector<ALevel_Data*>::size_type)level_number > Levels_Data.size())
		AsConfig::Throw();

	Current_Level_Number = level_number;

	level_data = Levels_Data[(int)level_number - 1];
...
{
	/*Return: true/false - finished clearing and displaying a new level/not yet
