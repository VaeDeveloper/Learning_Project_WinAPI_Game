...
AActive_Brick_Unbreakable::AActive_Brick_Unbreakable(int level_x, int level_y)
	: AActive_Brick(EBrick_Type::Unbreakable, level_x, level_y), Animation_Step(0), 
	Region(CreateRoundRectRgn(Brick_Rect.left, Brick_Rect.top, Brick_Rect.right + 1,
	Brick_Rect.bottom + 1, 2 * AsConfig::Global_Scale - 1, 2 * AsConfig::Global_Scale - 1))
{
}
//------------------------------------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------------------------------------
void AActive_Brick_Multihit::Draw_Stage(HDC hdc,const RECT& brick_rect, int x, int width)
...
AAdvertisement::AAdvertisement(int level_x, int level_y, int width, int height)
	: Level_X(level_x), Level_Y(level_y), Width(width), Height(height), Empty_Region(CreateRectRgn(0, 0, 0, 0)), Ball_X(0), Ball_Y(0),
	Ball_Width(Ball_Size* AsConfig::Global_Scale), Ball_Height(Ball_Size* AsConfig::Global_Scale), Ball_Y_Offset(0),
	Falling_Speed(0.0), Acceleration_Step(0.2), Brick_Regions(0), Deformation_Ratio(0)
{
	const int scale = AsConfig::Global_Scale;
